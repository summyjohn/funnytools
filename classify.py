...
DEFAULT_KEY = 'default'
BACKUP_FILE = '.backup.json' # 防止迁移的时候出现文件重名的情况 def unique_covert(file_path):
    new_path = '|'.join(file_path.split(os.sep)[:-1])
    if not new_path:
        return file_path
    return os.path.basename(file_path) + ' (' + new_path + ')' def coroutine(gen): def wrapper(*arg, **kws):
        coroutine = gen(*arg, **kws)
        next(coroutine)
        return coroutine
    return wrapper

@coroutine def save_back_up(target_dir):
...
# 按扩展名分类 def classify_by_ext(target_dir, tmp_dir): from collections import defaultdict
    ...
    yield None # 按修改时间分类 def classify_by_mtime(target_dir, tmp_dir): for dir_ in os.walk(target_dir):
        base_dir = dir_[0]
        for f in dir_[2]:
    ...
    yield None # 按字母分类 def classify_by_first_letter(target_dir, tmp_dir): for dir_ in os.walk(target_dir):
        base_dir = dir_[0]
        for f in dir_[2]:
            abs_file_path = os.path.join(base_dir, f)
            rel_file_path = os.path.relpath(abs_file_path, target_dir)
    ...
    yield None def go_back(target_dir):
